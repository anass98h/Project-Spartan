...
void NameChanger::BeginFrame(float frameTime)
{
	...
	if (changes >= 5)
	{
		switch (NameChanger::type)
		{
			case NC_Type::NC_NORMAL:
				SetName(Util::PadStringRight(XORSTR("\230Project Spartan"), strlen(XORSTR("\230Project Spartan")) + Util::RandomInt(10, 50)));
				break;
	...
	SetName(Util::PadStringRight(XORSTR("Project Spartan"), strlen(XORSTR("Project Spartan")) + changes));
}