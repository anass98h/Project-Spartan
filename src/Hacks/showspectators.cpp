    ...
    if ( ImGui::Begin( XORSTR( "Spectators" ), &Settings::ShowSpectators::enabled,
        ...
        for ( int playerId : ShowSpectators::GetObservervators( engine->GetLocalPlayer( ) ) ) {
            ...
            switch ( *player->GetObserverMode( ) ) {
                case ObserverMode_t::OBS_MODE_IN_EYE: ImGui::Text( XORSTR( "Perspective" ) );
                    break;
                case ObserverMode_t::OBS_MODE_CHASE: ImGui::Text( XORSTR( "3rd person" ) );
                    break;
                case ObserverMode_t::OBS_MODE_ROAMING: ImGui::Text( XORSTR( "Free look" ) );
                    break;
                case ObserverMode_t::OBS_MODE_DEATHCAM: ImGui::Text( XORSTR( "Deathcam" ) );
                    break;
                case ObserverMode_t::OBS_MODE_FREEZECAM: ImGui::Text( XORSTR( "Freezecam" ) );
                    break;
                case ObserverMode_t::OBS_MODE_FIXED: ImGui::Text( XORSTR( "Fixed" ) );
                    break;
                default: break;
            }
            ImGui::NextColumn( );
