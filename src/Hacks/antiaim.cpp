...
static float GetBestHeadEdgeAngle() {
    C_BasePlayer *localplayer = (C_BasePlayer *) entityList->GetClientEntity(engine->GetLocalPlayer());
...
bool NextLBYUpdate() {
    ...
    float lastLbyUpdate;
    bool firstBreak;
    bool shouldFirstBreak = true;
    if(LocalPlayer->GetVelocity().Length2D() == 0) {
        lastLbyUpdate = globalVars->curtime;
        if(lastLbyUpdate + 1.32 < flServerTime){
         //lastLbyUpdate = globalVars->curtime;
            return true;
        }
        return false;
...
static void DoAntiAimY(QAngle &angle, int command_number, bool bFlip, bool &clamp) {
    ...
    switch (antiAim) {
        ...
        case AntiAimType_Y::RASP:
            ...
            realAngle = AntiAim::lastRealYaw;
            if (Settings::AntiAim::Standing::Yaw::typeFake == AntiAimType_Y::RASP ||
                Settings::AntiAim::Moving::Yaw::typeFake == AntiAimType_Y::RASP ||
                Settings::AntiAim::Airborne::Yaw::typeFake == AntiAimType_Y::RASP) {
