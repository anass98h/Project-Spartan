...
static void DoAntiAimY( QAngle &angle, int command_number, bool bFlip, bool &clamp ) {
    ...
    switch ( aa_type ) {
        case AntiAimType_Y::LEGITTROLLING:Settings::AntiAim::Yaw::typeFake = AntiAimType_Y::NOAA;
            angle.y -= 180.0f;
            //Settings::AntiAim::Yaw::typeFake = AntiAimType_Y::NOAA;
            break;
        case AntiAimType_Y::LEGITTROLLING2:Settings::AntiAim::Yaw::typeFake = AntiAimType_Y::NOAA;
            angle.y -= 90.0f;
            //	Settings::AntiAim::Yaw::typeFake = AntiAimType_Y::NOAA;
            break;
        case AntiAimType_Y::SPIN:factor = 360.0 / M_PHI;
        ...
        case AntiAimType_Y::Tank:static bool fakeantiaim;
            ...
            break;
        case AntiAimType_Y::TANK2:static bool masturbation;
            ...
            break;
        case AntiAimType_Y::TANK3:static bool uff = false;
            ...
            if ( pLocal->GetVelocity( ).Length2D( ) < 0.1f ) {
                if ( CreateMove::sendPacket ) {
                    ...
                    if ( pLocal->GetFlags( ) & FL_ONGROUND )
                        ...
                        random = rand( ) % 4;
                        switch ( random ) {
                            case 1:yFlip ? angle.y += 90.f : angle.y -= 90.0f;
                                break;
                            case 2:yFlip ? angle.y -= 120.0f : angle.y -= 210.0f;
                                break;
                            case 3:factor = 360.0 / M_PHI;
                                factor *= 25;
                                angle.y = fmodf( globalVars->curtime * factor, 360.0 );
                                break;
                            default:angle.y -= 180.0f;
                        }
                    }
                    CreateMove::sendPacket = false;
                } else {
        ...
        case AntiAimType_Y::LBYBREAK:
            ...
            break;
        case AntiAimType_Y::LBYSPIN:factor = 360.0 / M_PHI;
            angle.y = *pLocal->GetLowerBodyYawTarget( ) + fmodf( globalVars->curtime * factor, 360.0 );
            break;
        case AntiAimType_Y::RANDOMBACKJITTER:angle.y -= 180;
            random = rand( ) % 100;
        ...
        case AntiAimType_Y::CASUALJITTER:yFlip ? angle.y -= 35.0f : angle.y += 35.0f;
            break;
        case AntiAimType_Y::LBYJITTER:
            ...
            break;
        case AntiAimType_Y::NOAA:break;
        case AntiAimType_Y::JITTER:yFlip ? angle.y -= 90.0f : angle.y -= 270.0f;
            break;
        case AntiAimType_Y::BACKJITTER:angle.y -= 180;
            random = rand( ) % 100;
            maxJitter = rand( ) % ( 85 - 70 + 1 ) + 70;
            temp = maxJitter - ( rand( ) % maxJitter );
            if ( random < 35 + ( rand( ) % 15 ) )
        ...
        case AntiAimType_Y::SIDEWAYSRIGHT:angle.y -= 90.0f;
            break;
        case AntiAimType_Y::SIDEWAYSLEFT:angle.y += 90.0f;
            break;
        ...
        case AntiAimType_Y::BACKWARDS:angle.y -= 180.0f;
            break;

        case AntiAimType_Y::FORWARDS:angle.y -= 0.0f;
            break;

        case AntiAimType_Y::STATICAA:angle.y = 0.0f;
            break;
        case AntiAimType_Y::STATICJITTER:trigger += 15.0f;
            angle.y = trigger > 50.0f ? 150.0f : -150.0f;

            if ( trigger > 100.0f )
                trigger = 0.0f;
            break;
        case AntiAimType_Y::STATICSMALLJITTER:trigger += 15.0f;
            angle.y = trigger > 50.0f ? -30.0f : 30.0f;

            if ( trigger > 100.0f )
                trigger = 0.0f;
            break;
        case AntiAimType_Y::LUA1:angle.y = LuaScriptY( lastAngleY, angle.y );
            break;
        case AntiAimType_Y::LUA2:angle.y = LuaScriptY2( lastAngleY2, angle.y );
            break;
        case AntiAimType_Y::LISP:clamp = false;

            yFlip ? angle.y += 323210000.0f : angle.y -= 323210000.0f;


            break;
        case AntiAimType_Y::LISP_SIDE:clamp = false;
            temp = angle.y + 90.0f;
            ...
            angle.y = temp;
            break;
        case AntiAimType_Y::LISP_JITTER:clamp = false;
            ...
            angle.y = temp;
            break;
        case AntiAimType_Y::ANGEL_BACKWARD:clamp = false;
            angle.y += 36000180.0f;
            break;
        case AntiAimType_Y::ANGEL_INVERSE:clamp = false;
            angle.y = 36000180.0f;
            break;
        case AntiAimType_Y::LOWERBODY:angle.y = *pLocal->GetLowerBodyYawTarget( ) + rand( ) % 35 + 165;
            break;

        case AntiAimType_Y::ANGEL_SPIN:clamp = false;
            factor = ( globalVars->curtime * 5000.0f );
            angle.y = factor + 36000000.0f;
            break;
        case AntiAimType_Y::CASUAL:yFlip ? angle.y -= 35.0f : angle.y += 35.0f;
            break;
        case AntiAimType_Y::LUA_UNCLAMPED:clamp = false;
            angle.y = LuaScriptY( lastAngleY, angle.y );
            break;
        case AntiAimType_Y::LUA_UNCLAMPED2:clamp = false;
...
static void DoAntiAimX( QAngle &angle, bool bFlip, bool &clamp ) {
    ...
    switch ( aa_type ) {
        case AntiAimType_X::STATIC_UP:angle.x = -89.0f;
            break;
        case AntiAimType_X::FLIP:angle.x = bFlip ? -55.0f : 40.0f;
            break;
        case AntiAimType_X::STATIC_DOWN:angle.x = 89.0f;
            break;

        case AntiAimType_X::DANCE:pDance += 45.0f;
            if ( pDance > 100 )
                pDance = 0.0f;
            else if ( pDance > 75.f )
                angle.x = -89.f;
                ...
                angle.x = 89.f;
            break;
        case AntiAimType_X::FRONT:angle.x = 0.0f;
            break;
        ...
        case AntiAimType_X::STATIC_DOWN_FAKE:
            ...
            break;
        case AntiAimType_X::LUA1:angle.x = LuaScriptX( lastAngleX, angle.x );
            break;
        case AntiAimType_X::LISP_DOWN:clamp = false;
            angle.x = 1800089.0f;
            break;
        case AntiAimType_X::ANGEL_DOWN:clamp = false;
            angle.x = 36000088.0f;
            break;
        case AntiAimType_X::ANGEL_UP:clamp = false;
            angle.x = 35999912.0f;
            break;
        case AntiAimType_X::LUA_UNCLAMPED:clamp = false;
            angle.x = LuaScriptX( lastAngleX, angle.x );
            break;
        default:angle.x -= 0.0f;
            break;
    }
    lastAngleX = angle.x;
}
...
static void DoAntiAimZ( QAngle &angle, int command_number, bool &clamp ) {
    ...
    switch ( aa_type ) {
        case AntiAimType_Z::REVERSE:angle.z = -50.0f;
            break;
        case AntiAimType_Z::AUTISMFLIP:angle.z = bSendPacket ? -50.0f : 45.0f;
            break;
...
static void DoAntiAimLBY( QAngle &angle, int command_number, bool bFlip, bool &clamp ) {
    ...
    switch ( aa_type ) {
        ...
        case AntiAimType_LBY::ONE:static bool flip1 = false;
            ...
            if ( pLocal->GetVelocity( ).x < 0.1f && pLocal->GetVelocity( ).x > -0.1f ) {
                ...
                angle.y -= 0;
            break;
        case AntiAimType_LBY::TWO:static bool flip2 = false;
            ...
            if ( pLocal->GetVelocity( ).x < 0.1f && pLocal->GetVelocity( ).x > -0.1f ) {
                ...
                angle.y -= 0.0f;
            break;
        case AntiAimType_LBY::THREE:static int flip3 = ( int )( floorf( globalVars->curtime ) / 1.1 ) % 2;
            ...
            if ( pLocal->GetVelocity( ).x < 0.1f && pLocal->GetVelocity( ).x > -0.1f )
                ...
                angle.y += 0.f;
            break;
        case AntiAimType_LBY::NONE:Settings::AntiAim::Lby::enabled = false;
            break;
    }
}
