...
static void DrawBox( Color color, int x, int y, int w, int h, C_BaseEntity* entity ) {
    ...
    } else if ( Settings::ESP::Boxes::type ==
                BoxType::HITBOXES ) /* credits to 1337floesen - https://www.unknowncheats.me/forum/counterstrike-global-offensive/157557-drawing-hitboxes.html */ {
...
static void DrawEntity( C_BaseEntity* entity, const char* string, Color color ) {
    ...
    if ( GetBox( entity, x, y, w, h ) ) {
        ...
        Draw::Text( ( int ) ( x + ( w / 2 ) - ( nameSize.x / 2 ) ), y + h + 2, string, esp_font,
    ...
    if ( player->SetupBones( pBoneToWorldOut, 128, 256, 0 ) ) {
        for ( int i = 0; i < pStudioModel->numbones; i++ ) {
            ...
            if ( debugOverlay->ScreenPosition(
                    Vector( pBoneToWorldOut[i][0][3], pBoneToWorldOut[i][1][3], pBoneToWorldOut[i][2][3] ),
                    vBonePos1 ) )
                continue;
            ...
            Draw::Line( Vector2D( vBonePos1.x, vBonePos1.y ), Vector2D( vBonePos2.x, vBonePos2.y ),
                        Color::FromImColor( Settings::ESP::Skeleton::color.Color() ) );
        }
    }
}
...
static void DrawAutoWall( C_BasePlayer* player ) {
    ...
    for ( int i = 0; i < 31; i++ ) {
        ...
        Draw::Text( Vector2D( bone2D.x, bone2D.y ), output.c_str(), autowallFont,
                    Color( 255, 0, 255, 255 ) ); // hot pink
    }
    matrix3x4_t matrix[128];
...
static void DrawHeaddot( C_BasePlayer* player ) {
    ...
    Draw::FilledCircle( Vector2D( head2D.x, head2D.y ), 10, Settings::ESP::HeadDot::size,
                        Color::FromImColor( ESP::GetESPPlayerColor( player, bIsVisible ) ) );
...
static void DrawPlayer( int index, C_BasePlayer* player, IEngineClient::player_info_t player_info ) {
    ...
    if ( Settings::ESP::Bars::enabled ) {
        ...
        if ( Settings::ESP::Bars::type == BarType::VERTICAL ) {
            ...
            if ( HealthPerc > 0 )
                Draw::FilledRectangle( barx + 1, bary + ( barh * ( 1.f - HealthPerc ) ) + 1, barx + barw - 1,
    ...
    if ( Settings::ESP::Info::name || Settings::ESP::Info::clan ) {
        ...
        Draw::Text( ( int ) ( x + ( w / 2 ) - ( nameSize.x / 2 ) ), ( int ) ( y - textSize.y - nameOffset ),
                    displayString.c_str(), esp_font, Color( 255, 255, 255 ) );
        multiplier++;
    }
    ...
    if ( Settings::ESP::Info::steamId ) {
        ...
        Draw::Text( ( int ) ( x + ( w / 2 ) - ( rankSize.x / 2 ) ),
                    ( int ) ( y - ( textSize.y * multiplier ) - nameOffset ), player_info.guid, esp_font,
                    Color( 255, 255, 255, 255 ) );
        multiplier++;
    }
    ...
    if ( Settings::ESP::Info::rank ) {
        ...
        if ( rank >= 0 && rank < 19 ) {
            ...
            Draw::Text( ( int ) ( x + ( w / 2 ) - ( rankSize.x / 2 ) ),
                        ( int ) ( y - ( textSize.y * multiplier ) - nameOffset ), ESP::ranks[rank], esp_font,
                        Color( 255, 255, 255, 255 ) );
        }
    }
    ...
    if ( Settings::ESP::Info::weapon && activeWeapon ) {
        ...
        Draw::Text( ( int ) ( x + ( w / 2 ) - ( weaponTextSize.x / 2 ) ), y + h + offset, modelName.c_str(), esp_font,
                    Color( 255, 255, 255 ) );
    }
    ...
    for ( auto Text : stringsToShow ) {
        Draw::Text( x + w + boxSpacing, ( int ) ( y + ( i * ( textSize.y + 2 ) ) ), Text.c_str(), esp_font,
                    Color( 255, 255, 255 ) );
        i++;
    }
...
static void DrawSounds() {
    for ( unsigned int i = 0; i < footsteps.size(); i++ ) {
        ...
        playerColor.a =
                std::min( powf( percent * 2, 0.6f ), 1.f ) * playerColor.a; // fades out alpha when its below 0.5
...
static void DrawFOVCrosshair() {
    ...
    if ( Settings::ESP::FOVCrosshair::filled )
        Draw::FilledCircle( Vector2D( width / 2, height / 2 ), 20, radius,
                            Color::FromImColor( Settings::ESP::FOVCrosshair::color.Color() ) );
    else
        Draw::Circle( Vector2D( width / 2, height / 2 ), 20, radius,
                      Color::FromImColor( Settings::ESP::FOVCrosshair::color.Color() ) );
...
static void DrawSpread() {
    ...
    if ( Settings::ESP::Spread::enabled ) {
        ...
        if ( cone > 0.0f ) {
            ...
            Draw::Rectangle( Vector2D( ( ( width / 2 ) - radius ), ( height / 2 ) - radius + 1 ),
                             Vector2D( ( width / 2 ) + radius + 1, ( height / 2 ) + radius + 2 ),
                             Color::FromImColor( Settings::ESP::Spread::color.Color() ) );
        }
    }
    if ( Settings::ESP::Spread::spreadLimit ) {
        ...
        if ( cone > 0.0f ) {
            ...
            Draw::Rectangle( Vector2D( ( ( width / 2 ) - radius ), ( height / 2 ) - radius + 1 ),
                             Vector2D( ( width / 2 ) + radius + 1, ( height / 2 ) + radius + 2 ),
                             Color::FromImColor( Settings::ESP::Spread::spreadLimitColor.Color() ) );
        }
    }
}
...
void ESP::Paint() {
    ...
    for ( int i = 1; i < entityList->GetHighestEntityIndex(); i++ ) {
        ...
        if ( ( client->m_ClassID != EClassIds::CBaseWeaponWorldModel &&
               ( strstr( client->m_pNetworkName, XORSTR( "Weapon" ) ) || client->m_ClassID == EClassIds::CDEagle ||
                 client->m_ClassID == EClassIds::CAK47 ) ) && Settings::ESP::Filters::weapons ) {
