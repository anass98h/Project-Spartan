...
static void LegitStrafe( C_BasePlayer *localplayer, CUserCmd *cmd ) {
    ...
    switch ( Settings::AutoStrafe::type ) {
        case AutostrafeType::AS_FORWARDS: cmd->sidemove = cmd->mousedx < 0.f ? -450.f : 450.f;
            break;
        case AutostrafeType::AS_BACKWARDS: cmd->sidemove = cmd->mousedx < 0.f ? 450.f : -450.f;
            break;
        case AutostrafeType::AS_LEFTSIDEWAYS: cmd->forwardmove = cmd->mousedx < 0.f ? -450.f : 450.f;
            break;
        case AutostrafeType::AS_RIGHTSIDEWAYS: cmd->forwardmove = cmd->mousedx < 0.f ? 450.f : -450.f;
            break;
        default: break;
    }
}
...
void AutoStrafe::CreateMove( CUserCmd *cmd ) {
    ...
    switch ( Settings::AutoStrafe::type ) {
        ...
        case AutostrafeType::AS_RIGHTSIDEWAYS: LegitStrafe( localplayer, cmd );
            break;
        case AutostrafeType::AS_RAGE: RageStrafe( localplayer, cmd );
            break;
    }
}
