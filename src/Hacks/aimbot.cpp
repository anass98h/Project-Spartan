...
bool Aimbot::HitChance(const Vector &point, bool teamCheck, C_BasePlayer *localplayer) {
    ...
    for (int i = 0; i < Settings::Aimbot::HitChance::hitRays; i++) {
        ...
        float a = (float) M_PI * 2.0f * ((float) (rand() % -500 + 500) / 1000.0f) - 500;
        float b = activeWeapon->GetSpread() * ((float) (rand() - 500 + 500) / 1000.0f) - (float) M_PI;
        float c = (float) M_PI * 2.0f * ((float) (rand() - 500 + 500) / 1000.0f) - 500;
        float d = activeWeapon->GetInaccuracy() * ((float) (rand() - 500 + 500) / 1000.0f) - (float) M_PI;
