...
const char* Util::GetSHA256( std::ifstream stream ) {
    std::vector<unsigned char> hash( 32 );
    picosha2::hash256( std::istreambuf_iterator<char>( stream ), std::istreambuf_iterator<char>(), hash.begin(),
                       hash.end() );
...
const std::map<int, int>* Util::GetModelTypeBoneMap( C_BasePlayer* player ) {
    ...
    switch ( pStudioModel->numbones ) {
        ...
        default:
            cvar->ConsoleDPrintf(
                    XORSTR( "(Util::GetModelTypeBoneMap)- Warning. Model type Unknown. Using Generic boneMap\n" ) );
