...
void HvH::RenderTab() {
    ...
    const char* yTypes[] = {
            "no aa", "Backwards", "Forwards", "FakeSideWays", "Spin", "Backjitter", "FJitter", "TJITTER", "Tank1",
            "Tank2", "richi's eap", "myrrib's eap",
            "lbyBreaker", "FakeLBY", "lbySpin", "LBYjitter", "LBY", "LBY + 180", "LEGITTROLLING", "LEGITTROLLING2",
            "Custom", "Custom2" // safe
    };
    ...
    const char* Hugtypes[] = {
            "OFF", "Tux", "Plus Delta", "PasteHub", "Brute Log EAP", "Bruteforce", "TapTipTopTupTep",
            "I'm feeling Lucky", "Myrrib's DELTA"
    };
    ...
    ImGui::NextColumn();
    {
        ImGui::BeginChild( XORSTR( "HVH2" ), ImVec2( 0, 0 ), true );
        {
            ...
            ImGui::Columns( 2, NULL, true );
            {
                ImGui::Checkbox( XORSTR( "Slowmotion" ),
                                 &Settings::SlowMo::enabled ); // why you move my memes ? :feelssad:
                ImGui::Checkbox( XORSTR( "Allow Untrusted Angles" ), &Settings::AntiAim::allowUntrustedAngles );
            }
            ImGui::NextColumn();
            {
