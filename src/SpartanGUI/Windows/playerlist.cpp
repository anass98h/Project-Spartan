    ...
    if ( ImGui::Begin( XORSTR( "Player list" ), &PlayerList::showWindow,
        ...
        if ( engine->IsInGame( ) && *csPlayerResource ) {
            ...
            for ( int team = ( int )TeamID::TEAM_UNASSIGNED; team <= ( int )TeamID::TEAM_COUNTER_TERRORIST; team++ ) {
                ...
                switch ( ( TeamID )team ) {
                    case TeamID::TEAM_UNASSIGNED:teamName = strdup( XORSTR( "Unassigned" ) );
                        break;
                    case TeamID::TEAM_SPECTATOR:teamName = strdup( XORSTR( "Spectator" ) );
                        break;
                    case TeamID::TEAM_TERRORIST:teamName = strdup( XORSTR( "Terrorist" ) );
                        break;
                    case TeamID::TEAM_COUNTER_TERRORIST:teamName = strdup( XORSTR( "Counter Terrorist" ) );
                        break;
                }
