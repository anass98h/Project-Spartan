...
serverName =
[
userDefaults objectForKey: SERVERNAME_KEY ];
self.
imgui =
[[
ImGuiHelper alloc
...
- (IBAction)onServernameTapped: ( id )sender
{
    ...
    alloc] initWithTitle:@"Set Server"
    message:@"Enter server name or IP for uSynergy"
    delegate:
    self
...
serverName forKey:SERVERNAME_KEY ];
[
...
@"Servername Updated"
message:
@"Restart the app to connect the server"
delegate:nil
cancelButtonTitle:
@"OK"
...
enabled = GL_TRUE;
self.effect.light0.
diffuseColor = GLKVector4Make( 1.0f, 0.4f, 0.4f, 1.0f );
...
_rotation += self.
timeSinceLastUpdate* (_hud
.rotation_speed * (M_PI / 180.0));
}
...
glUniformMatrix4fv(uniforms[UNIFORM_MODELVIEWPROJECTION_MATRIX],
1, 0, _modelViewProjectionMatrix.m);
glUniformMatrix3fv(uniforms[UNIFORM_NORMAL_MATRIX],
1, 0, _normalMatrix.m);
glUniform3f(uniforms[UNIFORM_DIFFUSE_COLOR], _hud
.cubeColor1[0], _hud.cubeColor1[1], _hud.cubeColor1[2] );
...
self.effect.light0.
diffuseColor = GLKVector4Make( _hud.cubeColor2[0], _hud.cubeColor2[1], _hud.cubeColor2[2], 1.0f );
...
file encoding:
NSUTF8StringEncoding error:nil] UTF8String];
if (!source) {
