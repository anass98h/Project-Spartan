...
uSynergyBool ImGui_ConnectFunc( uSynergyCookie cookie ) {
    ...
    // get server address
    getaddrinfo([g_serverName
    UTF8String], "24800", &hints, &res);
...
//    printf("Synergy: keyboard callback: 0x%02X (%s)", scanCode, down?"true":"false");
    ...
    if ( ( down ) && ( key ) && ( scanCode < 256 ) && !( modifiers & USYNERGY_MODIFIER_CTRL ) ) {
        ...
        int charForKeycode = ( modifiers & USYNERGY_MODIFIER_SHIFT ) ? g_keycodeCharShifted[scanCode]
                                                                     : g_keycodeCharUnshifted[scanCode];
...
void
ImGui_JoystickCallback( uSynergyCookie cookie, uint8_t joyNum, uint16_t buttons, int8_t leftStickX, int8_t leftStickY,
                        int8_t rightStickX, int8_t rightStickY ) {
    printf( "Synergy: joystick callback TODO\n" );
}

void ImGui_ClipboardCallback( uSynergyCookie cookie, enum uSynergyClipboardFormat format, const uint8_t* data,
                              uint32_t size ) {
...
style.
TouchExtraPadding = ImVec2( 4.0, 4.0 );

io.
RenderDrawListsFn = ImGui_ImplIOS_RenderDrawLists;
...
_synergyCtx.
m_clientWidth = self.view.bounds.size.width;
_synergyCtx.
m_clientHeight = self.view.bounds.size.height;

_synergyCtx.
m_connectFunc = ImGui_ConnectFunc;
_synergyCtx.
m_sendFunc = ImGui_SendFunc;
_synergyCtx.
m_receiveFunc = ImGui_RecvFunc;
_synergyCtx.
m_sleepFunc = ImGui_SleepFunc;
_synergyCtx.
m_traceFunc = ImGui_TraceFunc;
_synergyCtx.
m_getTimeFunc = ImGui_GetTimeFunc;

_synergyCtx.
m_traceFunc = ImGui_TraceFunc;
_synergyCtx.
m_screenActiveCallback = ImGui_ScreenActiveCallback;
_synergyCtx.
m_mouseCallback = ImGui_MouseCallback;
_synergyCtx.
m_keyboardCallback = ImGui_KeyboardCallback;

_synergyCtx.
m_cookie = ( uSynergyCookie ) & _synergyCtx;
...
- (void)
viewDidPan: ( UIPanGestureRecognizer * )
recognizer
{
...
- (void)
viewDidTap: ( UITapGestureRecognizer * )
recognizer
{
_touchPos =
[
...
io.
DisplaySize = ImVec2( _view.bounds.size.width, _view.bounds.size.height );

io.
MouseDrawCursor = g_synergyPtrActive;
if (g_synergyPtrActive)
{
style.
TouchExtraPadding = ImVec2( 0.0, 0.0 );
io.
MousePos = ImVec2( g_mousePosX, g_mousePosY );
for (
int i = 0;
i < 3; i++)
{
...
io.
MouseWheel = g_mouseWheelY / 500.0;
}
else
{
...
TouchExtraPadding = ImVec2( 4.0, 4.0 );
io.
MousePos = ImVec2( _touchPos.x, _touchPos.y );
...
static void ImGui_ImplIOS_RenderDrawLists( ImDrawData* draw_data ) {
    ...
    for ( int n = 0; n < draw_data->CmdListsCount; n++ ) {
        ...
        unsigned char* vtx_data = ( unsigned char* ) glMapBufferRange( GL_ARRAY_BUFFER, 0, needed_vtx_size,
                                                                       GL_MAP_WRITE_BIT |
