...
ImVec2 ImGui::CalcItemSize( ImVec2 size, float default_x, float default_y ) {
    ...
    if ( size.x <= 0.0f )
        size.x = ( size.x == 0.0f ) ? default_x : ImMax( content_max.x - g.CurrentWindow->DC.CursorPos.x, 4.0f ) +
                                                  size.x;
    if ( size.y <= 0.0f )
        size.y = ( size.y == 0.0f ) ? default_y : ImMax( content_max.y - g.CurrentWindow->DC.CursorPos.y, 4.0f ) +
                                                  size.y;
    return size;
}
...
void ImGui::NewFrame() {
    ...
    g.IO.WantCaptureKeyboard = ( g.CaptureKeyboardNextFrame != -1 ) ? ( g.CaptureKeyboardNextFrame != 0 ) : (
            g.ActiveId !=
            0 );
...
static void BeginTooltipEx( bool override_previous_tooltip ) {
    ...
    ImGui::Begin( window_name, NULL, ImGuiWindowFlags_Tooltip | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoMove |
...
static ImVec2
FindBestPopupWindowPos( const ImVec2& base_pos, const ImVec2& size, int* last_dir, const ImRect& r_inner ) {
    ...
    for ( int n = ( *last_dir != -1 ) ? -1 : 0;
          n < 4; n++ ) // Last, Right, down, up, left. (Favor last used direction).
    {
...
bool
ImGui::Begin( const char* name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha,
              ImGuiWindowFlags flags ) {
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    ...
    if ( first_begin_of_the_frame ) {
        ...
        if ( window_pos_center ) {
            ...
            SetWindowPos( window,
                          ImMax( style.DisplaySafeAreaPadding, fullscreen_rect.GetCenter() - window->SizeFull * 0.5f ),
                          0 );
        ...
        if ( window->ScrollTarget.y < FLT_MAX ) {
            ...
            window->Scroll.y = window->ScrollTarget.y -
                               ...
                               ( center_ratio * ( window->SizeFull.y - window->ScrollbarSizes.y ) );
        ...
        if ( window->Collapsed ) {
            ...
            if ( !( flags & ImGuiWindowFlags_NoTitleBar ) )
                window->DrawList->AddRectFilled( title_bar_rect.GetTL(), title_bar_rect.GetBR(), GetColorU32(
                        ( g.NavWindow && window->RootNonPopupWindow == g.NavWindow->RootNonPopupWindow )
            ...
            if ( !( flags & ImGuiWindowFlags_NoResize ) ) {
                ...
                window->DrawList->PathArcToFast( ImVec2( br.x - window_rounding - window->BorderSize,
                                                         br.y - window_rounding - window->BorderSize ), window_rounding,
                                                 0,
                                                 3 );
...
void ImGui::SetScrollHere( float center_y_ratio ) {
    ...
    target_y +=
            ( window->DC.PrevLineHeight * center_y_ratio ) + ( GImGui->Style.ItemSpacing.y * ( center_y_ratio - 0.5f ) *
...
bool ImGui::TreeNodeBehavior( ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end ) {
    ...
    if ( display_frame ) {
        ...
        if ( flags & ImGuiTreeNodeFlags_Bullet )
            ...
            RenderCollapseTriangle( bb.Min + ImVec2( padding.x, g.FontSize * 0.15f + text_base_offset_y ), is_open,
                                    0.70f );
        if ( g.LogEnabled )
...
bool
ImGui::SliderFloat2( const char* label, float v[2], float v_min, float v_max, const char* display_format,
                     float power ) {
    return SliderFloatN( label, v, 2, v_min, v_max, display_format, power );
}

bool
ImGui::SliderFloat3( const char* label, float v[3], float v_min, float v_max, const char* display_format,
                     float power ) {
    return SliderFloatN( label, v, 3, v_min, v_max, display_format, power );
}

bool
ImGui::SliderFloat4( const char* label, float v[4], float v_min, float v_max, const char* display_format,
                     float power ) {
...
bool ImGui::DragBehavior( const ImRect& frame_bb, ImGuiID id, float* v, float v_speed, float v_min, float v_max,
    ...
    const ImU32 frame_col = GetColorU32(
            g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered
                                                                          : ImGuiCol_FrameBg );
...
void
ImGui::PlotEx( ImGuiPlotType plot_type, const char* label, float (* values_getter)( void* data, int idx ), void* data,
...
void
ImGui::PlotHistogram( const char* label, float (* values_getter)( void* data, int idx ), void* data, int values_count,
                      int values_offset, const char* overlay_text, float scale_min, float scale_max,
                      ImVec2 graph_size ) {
...
#define STB_TEXTEDIT_MOVEWORDRIGHT  STB_TEXTEDIT_MOVEWORDRIGHT_IMPL
    ...
    static bool
    STB_TEXTEDIT_INSERTCHARS( STB_TEXTEDIT_STRING* obj, int pos, const ImWchar* new_text, int new_text_len ) {
...
ImGui::InputTextMultiline( const char* label, char* buf, size_t buf_size, const ImVec2& size, ImGuiInputTextFlags flags,
                           ImGuiTextEditCallback callback, void* user_data ) {
    return InputTextEx( label, buf, ( int ) buf_size, size, flags | ImGuiInputTextFlags_Multiline, callback,
                        user_data );
}
...
bool
ImGui::ListBox( const char* label, int* current_item, bool (* items_getter)( void*, int, const char** ), void* data,
                int items_count, int height_in_items ) {
...
bool ImGui::BeginMenu( const char* label, bool enabled ) {
    ...
    if ( window->DC.LayoutType == ImGuiLayoutType_Horizontal ) {
        ...
        RenderCollapseTriangle( pos + ImVec2( window->MenuColumns.Pos[2] + extra_w + g.FontSize * 0.20f, 0.0f ),
                                false );
...
void ImGui::SetColorEditOptions( ImGuiColorEditFlags flags ) {
    ...
    IM_ASSERT(
            ImIsPowerOfTwo( ( int ) ( flags & ImGuiColorEditFlags__InputsMask ) ) ); // Check only 1 option is selected
    IM_ASSERT( ImIsPowerOfTwo(
            ( int ) ( flags & ImGuiColorEditFlags__DataTypeMask ) ) ); // Check only 1 option is selected
    IM_ASSERT(
            ImIsPowerOfTwo( ( int ) ( flags & ImGuiColorEditFlags__PickerMask ) ) ); // Check only 1 option is selected
...
bool ImGui::ColorButton( const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size ) {
    ...
    if ( ( flags & ImGuiColorEditFlags_AlphaPreviewHalf ) && col.w < 1.0f ) {
        ...
        RenderColorRectWithAlphaCheckerboard( ImVec2( bb.Min.x + grid_step, bb.Min.y ), bb.Max, GetColorU32( col ),
                                              grid_step, ImVec2( -grid_step, 0.0f ), rounding,
                                              ImGuiCorner_TopRight | ImGuiCorner_BottomRight );
        window->DrawList->AddRectFilled( bb.Min, ImVec2( mid_x, bb.Max.y ), GetColorU32( col_without_alpha ), rounding,
                                         ImGuiCorner_TopLeft | ImGuiCorner_BottomLeft );
    } else {
        RenderColorRectWithAlphaCheckerboard( bb.Min, bb.Max, GetColorU32(
                ( flags & ImGuiColorEditFlags_AlphaPreview ) ? col : col_without_alpha ), grid_step, ImVec2( 0, 0 ),
                                              rounding );
    }
    if ( window->Flags & ImGuiWindowFlags_ShowBorders )
...
static void ColorEditOptionsPopup( ImGuiColorEditFlags flags ) {
    ...
    if ( allow_opt_datatype ) {
        ...
        if ( ImGui::RadioButton( "0..255", ( opts & ImGuiColorEditFlags_Uint8 ) ? 1 : 0 ) )
            opts = ( opts & ~ImGuiColorEditFlags__DataTypeMask ) | ImGuiColorEditFlags_Uint8;
        if ( ImGui::RadioButton( "0.00..1.00", ( opts & ImGuiColorEditFlags_Float ) ? 1 : 0 ) )
...
static void ColorPickerOptionsPopup( ImGuiColorEditFlags flags, float* ref_col ) {
    ...
    ImGuiContext& g = *GImGui;
    if ( allow_opt_picker ) {
        ImVec2 picker_size( g.FontSize * 8, ImMax( g.FontSize * 8 - ( ColorSquareSize() + g.Style.ItemInnerSpacing.x ),
...
bool ImGui::ColorEdit4( const char* label, float col[4], ImGuiColorEditFlags flags ) {
    ...
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    const float w_extra = ( flags & ImGuiColorEditFlags_NoSmallPreview ) ? 0.0f : ( ColorSquareSize() +
                                                                                    style.ItemInnerSpacing.x );
    ...
    if ( ( flags & ( ImGuiColorEditFlags_RGB | ImGuiColorEditFlags_HSV ) ) != 0 &&
        ...
        const float w_item_one = ImMax( 1.0f,
                                        ( float ) ( int ) (
                                                ( w_items_all - ( style.ItemInnerSpacing.x ) * ( components - 1 ) ) /
                                                ( float ) components ) );
...
static void RenderArrowsForVerticalBar( ImDrawList* draw_list, ImVec2 pos, ImVec2 half_sz, float bar_w ) {
    RenderArrow( draw_list, ImVec2( pos.x + half_sz.x + 1, pos.y ), ImVec2( half_sz.x + 2, half_sz.y + 1 ),
                 ImGuiDir_Right,
                 IM_COL32_BLACK );
...
static void
PaintVertsLinearGradientKeepAlpha( ImDrawVert* vert_start, ImDrawVert* vert_end, ImVec2 gradient_p0, ImVec2 gradient_p1,
...
bool ImGui::ColorPicker4( const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col ) {
    ...
    const ImU32 hue_colors[
            6 + 1] = { IM_COL32( 255, 0, 0, 255 ), IM_COL32( 255, 255, 0, 255 ), IM_COL32( 0, 255, 0, 255 ),
...
void ImGui::SetColumnOffset( int column_index, float offset ) {
    ...
    if ( !( window->DC.ColumnsFlags & ImGuiColumnsFlags_NoForceWithinWindow ) )
        offset = ImMin( offset,
                        ...
                        g.Style.ColumnsMinSpacing * ( window->DC.ColumnsCount - column_index ) );
    ...
    if ( ImGui::BeginPopup( popup_id ) ) {
        ...
        for ( int item_n = 0; item_n < items_count; item_n++ ) {
            ...
            draw_list->PathFillConvex(
                    hovered ? ImColor( 13, 13, 13 ) : /* selected ? ImColor(170,170,215) :*/ ImColor( 9, 9, 9 ) );
